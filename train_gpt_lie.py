...
class Q(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        self.orthogonal_basis = nn.Parameter(torch.randn(dim, dim))

    def forward(self, x):
        # orthogonal basis is parameterized by Householder reflections
        return self.orthogonal_basis @ x
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = self.orthogonal_basis(q)
        k = self.orthogonal_basis(k)
